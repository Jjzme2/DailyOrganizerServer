component name="Logger" {
	...
	// ----------------------------------------------------------- Objects/Models ----------------------------------------------------------- //
		...
		private string function getDirectory(prefix) {
			prefix = right(prefix, 1) eq "/" ? prefix : prefix & "/";
		...
		private string function getFileName(directory) {
			var fileName = arguments.directory & "Log_#dateFormat(now(), "yyyymmdd")#.#variables.fileType#";

			if(!fileExists(fileName)){
				writeOutput("Creating file: " & fileName);
				fileWrite(expandPath(fileName), '');
			}
	...
	// ----------------------------------------------------------- Log Entry Types ----------------------------------------------------------- //
		private struct function newLogEntrySimple(required includeTimestamp, string prefix) {
		...
		private struct function newLogEntryWithProperties(required struct properties, required includeTimestamp, string prefix) {
			return {
				"message"   	: "",
					"prefix" : isNull(arguments.prefix) ? "UNDEFINED" : arguments.prefix,
				"timestamp" 	: Now(),
	...
	// ----------------------------------------------------------- Private Functions ----------------------------------------------------------- //
		...
		private void function write(
			...
			try {
				...
				var file = fileOpen(logFileName, "append");
				var logMessage = preLogMessage & logEntryJSON & postLogMessage;

				// Write the log entry to the file
				fileWriteLine(file, separator);
				fileWriteLine(file, logMessage);
	...
	// ----------------------------------------------------------- Logging Functions ----------------------------------------------------------- //
		public void function sendSimpleLog(required string message, required string prefix, hasTimeStamp=false) {
		...
		public void function sendDetailedLog(required string message, required struct additionalInfo={}, required string prefix) {
			var preLogMessage = "";
			var postLogMessage = "";
