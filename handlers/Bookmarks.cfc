...
component extends="BaseApi" {
	...
	property name="dataServer" 	    inject="bookmarkServer";
	...
	remote function getBookmark( event, rc, prc ) {
	...
	remote function searchBookmarksForYoutubeFromJournal( event, rc, prc ) {
		var target = dataServer.getYoutubeURLs( rc.youtube, rc.journalID );
		event.renderData( type="json", data=target );
	}

	remote function searchBookmarkUrlsFromJournalContaining ( event, rc, prc ) {
		var target = dataServer.getBySearchTerm( rc.searchTerm, rc.journalID );
		event.renderData( type="json", data=target );
	}

	remote function store ( event, rc, prc ) {
